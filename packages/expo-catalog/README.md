...
<a href="https://www.npmjs.com/package/expo-catalog"><img src="https://img.shields.io/npm/v/expo-catalog.svg" alt="npm version"></a>
...
version strings. `expo-catalog` temporarily resolves the catalog references across your **entire**
pnpm workspace, runs the real Expo tooling against the resolved manifest, and restores the original
`package.json` afterwards.

## Install

```bash
npm install --save-dev expo-catalog
```
...
```sh
npx expo-catalog check --app-dir apps/expo
npx expo-catalog sync --app-dir apps/expo
```

```json
{
  "scripts": {
    "catalog:check": "expo-catalog check --app-dir apps/expo",
    "catalog:sync": "expo-catalog sync --app-dir apps/expo"
  }
}
```
...
```
Usage: expo-catalog check [options]
...
```
Usage: expo-catalog sync [options]
...
import { findWorkspaceRoot, runCheck, runSync } from "expo-catalog";
...
By default, `expo-catalog` derives the top-level workspace directories to scan (e.g. `apps`,
