...
shares an Expo-managed dependency) to its concrete version, runs `expo-doctor` against that resolved
manifest, then restores the original `package.json`, pass or fail.
